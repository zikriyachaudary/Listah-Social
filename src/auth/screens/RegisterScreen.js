...
import { Alert, Image, Modal, SafeAreaView, ScrollView, StyleSheet, Text, TouchableOpacity, TouchableWithoutFeedback } from 'react-native';
  ...
  const _handleSubmit = async () => {
    ...
    if (!disabled) {
      if (regex.test(password)) {
        ...
        setLoading(true)
        const compressedImage = await ImageResizer.createResizedImage(image.uri, 1000, 1000, 'PNG', 100, 0);
          ...
          <Text style={{
            ...
            <Text
              ...
              style={{
                ...
                fontWeight: "bold",
                color: "black"
              }}>{"Terms and Condition"}</Text>{" and acknowledge"}</Text>
        </View>
        <View center>
          <Button
            title='Sign up'
