...
export const suggestPost = (suggestion, cb) => async (dispatch) => {
  try {
    ...
    const authorProfile = await (await ProfileCollection.doc(suggestion?.authorId).get()).data();
    ...
    const updatedAuthorProfile = await (await ProfileCollection.doc(suggestion?.authorId).get()).data();
...
export const suggestApprove = (suggestion, cb) => async (dispatch) => {
  try {
    ...
    const suggestionPost = await (await PostsCollection.doc(postId).get()).data();
    ...
    if (change?.type === 'delete') {
      dispatch(updatePost({
        ...
        id: postId,
        items: suggestionPost?.items.filter((item) => item?.id !== change?.item?.id)
      }))
    }
